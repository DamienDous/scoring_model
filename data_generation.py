...
def parse_arguments():
	...
	parser.add_argument("in_data_path",
						help="Path to data credit tables.",
						type=str)
	parser.add_argument("out_path",
						help="Path where global data is write.",
						type=str)
	# Print version
...
class data_generation:
	...
	def __one_hot_encoder(self, df, nan_as_category=True):
...
if __name__ == "__main__":
	...
	with timer("Read credits tables data"):
		dataframes = {}
		for table_name in tables_names:
			dataframes[table_name] = pd.read_csv(
				args.in_data_path+'/'+table_name+'.csv')

	with timer("Global dataframe generation"):
		dg = data_generation()
		df = dg.process_database(dataframes)
		print(len(df.columns))
		print(len(df))

	with timer("Save global dataframe"):
		df.to_csv(args.out_path+'/global_test_data.csv')
